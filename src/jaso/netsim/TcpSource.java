...
public class TcpSource implements Sink, Handler {
	...
	enum State {
		IDLE, SLOW_START, CONG_AVOID, FAST_RECOV, FINISH
	}
	...
	public TcpSource(Host host, long flowId, int dst, long flowSize) {
		...
		if (flowSize < 1)
			throw new RuntimeException("flowSize must be greater than zero");
...
//	        }
		...
		if (packet.ackSeqNum < lastAcked) {
			System.out.println(
					"TCP " + "ACK from the past: packet.ackSeqNum " + packet.ackSeqNum + " lastAcked " + lastAcked);
			return;
		}
		...
		if (lastAcked < _recover_seq) {
			// See RFC 3782: if we haven't recovered from timeouts etc. don't do fast
			// recovery.
			return;
		}
